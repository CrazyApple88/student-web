package com.xhgx.commons.redis;
...
import com.xhgx.commons.properties.ProUtils;
...
public class RedisUtils {
	...
	private static final int JEDISPOOLCONFIG_MAXTOTAL;

	private static final int JEDISPOOLCONFIG_MAXIDLE;

	private static final Long JEDISPOOLCONFIG_MAXWAITMILLIS;

	private static final boolean JEDISPOOLCONFIG_TESTONBORROW;
	...
	/** 操作Key的方法 */
	public RedisKeysUtils keys;
	/** 对存储结构为String类型的操作 */
	public RedisStringsUtils strings;
	/** 对存储结构为List类型的操作 */
	public RedisListsUtils lists;
	/** 对存储结构为Set类型的操作 */
	public RedisSetsUtils sets;
	/** 对存储结构为HashMap类型的操作 */
	public RedisHashUtils hash;
	/** 对存储结构为Set(排序的)类型的操作 */
	public RedisSortSetUtils sortset;
	...
	static {
		...
		if (ProUtils.getValue("jedisPoolConfig.maxTotal") == null
				|| ProUtils.getValue("jedisPoolConfig.maxIdle") == null
				|| ProUtils.getValue("jedisPoolConfig.maxWaitMillis") == null
				|| ProUtils.getValue("jedisPoolConfig.testOnBorrow") == null
				|| ProUtils.getValue("redis.host") == null
				|| ProUtils.getValue("redis.port") == null
				|| ProUtils.getValue("redis.password") == null) {
			throw new RuntimeException(
					"未加载配置文件，可通过ProUtils.getProperties()或ProUtils.getGlobleProperties()加载配置文件");
		} else {
			JEDISPOOLCONFIG_MAXTOTAL = Integer.parseInt(ProUtils
					.getValue("jedisPoolConfig.maxTotal"));
			JEDISPOOLCONFIG_MAXIDLE = Integer.parseInt(ProUtils
					.getValue("jedisPoolConfig.maxIdle"));
			JEDISPOOLCONFIG_MAXWAITMILLIS = Long.parseLong(ProUtils
					.getValue("jedisPoolConfig.maxWaitMillis"));
			JEDISPOOLCONFIG_TESTONBORROW = Boolean.parseBoolean(ProUtils
					.getValue("jedisPoolConfig.testOnBorrow"));
			REDIS_HOST = ProUtils.getValue("redis.host");
			REDIS_PORT = Integer.parseInt(ProUtils.getValue("redis.port"));
			REDIS_PASSWORD = ProUtils.getValue("redis.password");
			// 控制一个pool可分配多少个jedis实例，通过pool.getResource()来获取；
			// 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
			config.setMaxTotal(JEDISPOOLCONFIG_MAXTOTAL);
			// 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
			config.setMaxIdle(JEDISPOOLCONFIG_MAXIDLE);
			// 表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException；
			config.setMaxWaitMillis(JEDISPOOLCONFIG_MAXWAITMILLIS);
			// 在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
			config.setTestOnBorrow(JEDISPOOLCONFIG_TESTONBORROW);
		}

		if ("".equals(REDIS_PASSWORD) || REDIS_PASSWORD == null) {
			jedisPool = new JedisPool(config, REDIS_HOST, REDIS_PORT);
		} else {
			jedisPool = new JedisPool(config, REDIS_HOST, REDIS_PORT, 10000,
					REDIS_PASSWORD);
		}
	}
	...
	public static Jedis getJedis() {
		if (jedisPool != null) {
			...
			throw new RuntimeException(
					"未加载配置文件，可通过ProUtils.getProperties('config.properties')加载配置文件");
		}

	}

	private static final RedisUtils REDIS_UTILS = new RedisUtils();
	...
	/**Keys*/
	public static class RedisKeysUtils {
	...
	/**Sets*/
	public static class RedisSetsUtils {
	...
	/**SortSet*/
	public static class RedisSortSetUtils {
	...
	/**Hash*/
	public static class RedisHashUtils {
	...
	/**Strings*/
	public static class RedisStringsUtils {
		/**
		 * 根据key获取记录
		 * 
		 * @param String
		...
		public static String set(String key, String value) {
			return RedisStringsUtils.set(SafeEncoder.encode(key),
	...
	/**Lists*/
	public static class RedisListsUtils {
		/**
		 * List长度
		 * 
		 * @param String
