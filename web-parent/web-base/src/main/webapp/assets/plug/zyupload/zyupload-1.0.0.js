...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
					...
					conselect        : "display:block",  				// 是否展示继续选择按钮
					selectinfo       : "display:block",  				// 是否展示选择文件大小信息
					selectinfo2      : "display:none",  				// 是否展示流程定义选择文件大小信息
					showFile         : "display:block",  				// 是否展示选择的文件
					/* 提供给外部的接口方法 */
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				<div id="status_info2" class="info2" style="'+para.selectinfo2+'">未选择流程定义文件。</div>';
		            html += '				<div id="status_info" class="info" style="'+para.selectinfo+'">选中0张文件，共0B。</div>';
		            html += '				<div class="btns">';
		            html += '					<div class="webuploader_pick" style="'+para.conselect+'">继续选择</div>';
		            html += '					<div class="upload_btn">'+para.buttonName+'</div>';
		            html += '				</div>';
		            html += '			</div>';
					html += '			<div id="preview" style="'+para.showFile+'" class="upload_preview"></div>';
					html += '		</div>';
		            ...
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div id="preview" style="'+para.showFile+'" class="upload_preview">';
			 ...
			 * 返回: 无
			 */
			this.funSetStatusInfo2 = function(files){
				var name = "";
				$.each(files, function(k,v){
					name = v.name;
				});
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						self.funSetStatusInfo2(ZYFILE.funReturnNeedFiles());
						var html = '', i = 0;
						// 组织预览html
