...
import com.xhgx.web.base.property.*;
...
/**
 * @ClassName AbstractAppController
 * @Description app端通用抽象controller层
 * @author zohan(inlw@sina.com)
 * @date 2017-03-24 9:18
 * @vresion 1.0
 */
@Controller("AbstractAppController")
@Scope("prototype")
public abstract class AbstractAppController extends AbstractBaseController implements ResultRender {

	static Logger log = LoggerFactory.getLogger(AbstractAppController.class);
	 ...
	 * @param headers
	 */
	public void renderStream(String contentType, InputStream inputStream,
			String... headers) {
		HttpServletResponse response = this.getResponse();
		try {
	...
	protected OnlineUser getCurrentUser() {
		OnlineUser user = (OnlineUser) this.getRequest().getSession()
				.getAttribute(OnlineUser.ONLINE_USER_TAG);
		if (user == null) {
